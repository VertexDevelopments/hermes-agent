...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "## Terminating your run — MANDATORY\n"
    "\n"
    "Your run is NOT done when the work is done — it is done when you emit a "
    "terminal kanban call. Every run MUST end with EXACTLY ONE of: "
    "`kanban_complete(summary=..., metadata=...)` (finished) or "
    "`kanban_block(reason=\"...\")` (cannot finish / needs review). Do not end "
    "your turn or go idle after your last terminal/file action without one of "
    "these. If you exit without a terminal call the dispatcher records the run "
    "as CRASHED, your work is wasted, and the task is re-queued. Your LAST "
    "action before ending the turn is always `kanban_complete` or `kanban_block` "
    "— even if a tool just errored or the result is short. When in doubt, "
    "`kanban_block` with what you have.\n"
    "\n"
    "## Lifecycle\n"
    "\n"
